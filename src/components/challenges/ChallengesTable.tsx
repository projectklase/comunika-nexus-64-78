...
export function ChallengesTable({ challenges, onEdit, onDelete, onToggleActive }: ChallengesTableProps) {
  ...
  const executeDelete = () => {
    if (deleteDialog.challengeId) {
      onDelete(deleteDialog.challengeId);
      setDeleteDialog({ isOpen: false, challengeId: null, challengeTitle: '' });
    }
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={`Tem certeza que deseja excluir "${deleteDialog.challengeTitle}"? O desafio será removido permanentemente.`}
        confirmText="Excluir"
        cancelText="Cancelar"
