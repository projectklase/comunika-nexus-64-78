...
-- Atualizar função attack para conceder XP automaticamente quando batalha termina
...
BEGIN
  ...
  -- DERROTA: Jogador atual com HP <= 0 (contra-ataque)
  ELSIF v_player_hp <= 0 THEN
    -- Determinar ID do vencedor (é o oponente)
    v_loser_id := p_player_id;
    
    -- Conceder XP: 50 para vencedor (oponente), 10 para perdedor (jogador atual)
  ...
  -- VERIFICAÇÃO: Vitória por deck-out (oponente sem cartas na mão E sem monstro no campo)
