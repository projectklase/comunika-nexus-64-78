-- Adiciona novos tipos de efeito TRAP: SWAP_STATS, BOOST, DRAIN
-- MANTÉM INTACTO: balanceamento FREEZE/HEAL por raridade, XP, overflow, summoning sickness, logs com logged_at
...
DECLARE
  ...
  v_logged_at TEXT;
  -- NOVAS VARIÁVEIS PARA EFEITOS ADICIONAIS
  v_temp_atk INT;
  v_temp_def INT;
  v_drain_amount INT;
  v_boost_amount INT;
    ...
    IF v_trap->'effects' IS NOT NULL THEN
      ...
      LOOP
        CASE v_effect->>'type'
          ...
          -- ===== NOVOS EFEITOS DE TRAP =====
          
          WHEN 'SWAP_STATS' THEN
            -- Troca ATK e DEF do monstro atacante (Espelho Reverso)
            v_temp_atk := COALESCE((v_attacker->>'atk')::INT, 0);
            v_temp_def := COALESCE((v_attacker->>'def')::INT, (v_attacker->>'current_hp')::INT, 0);
            ...
            v_log_entry := jsonb_build_object(
              ...
              'old_atk', v_temp_atk,
              'new_atk', v_temp_def,
              'message', '🪞 ' || v_trap_name || ' ativada! ATK e DEF de ' || (v_attacker->>'name') || ' foram trocados! (' || v_temp_atk || ' → ' || v_temp_def || ')',
              'logged_at', v_logged_at,
              'timestamp', NOW()
            );
            v_battle_log := v_battle_log || v_log_entry;
          ...
          WHEN 'BOOST' THEN
            -- Dá ATK ao monstro defensor (Emboscada)
            v_boost_amount := COALESCE((v_effect->>'value')::INT, 20);
            ...
            v_log_entry := jsonb_build_object(
              ...
              'trap', v_trap_name,
              'effect', 'BOOST',
              'value', v_boost_amount,
              'message', '⚔️ ' || v_trap_name || ' ativada! +' || v_boost_amount || ' ATK ao monstro defensor!',
              'logged_at', v_logged_at,
              'timestamp', NOW()
            );
            v_battle_log := v_battle_log || v_log_entry;
          ...
          WHEN 'DRAIN' THEN
            -- Drena HP: danifica atacante E cura dono da trap (Roubo de Energia)
            v_drain_amount := COALESCE((v_effect->>'value')::INT, 10);
            -- Danifica HP do jogador atacante
            v_my_hp := COALESCE((v_game_state->>(v_player_key || '_hp'))::INT, 100);
            v_my_hp := GREATEST(0, v_my_hp - v_drain_amount);
            v_game_state := jsonb_set(v_game_state, ARRAY[v_player_key || '_hp'], to_jsonb(v_my_hp));
            -- Cura HP do dono da trap (oponente)
            v_opponent_hp := COALESCE((v_game_state->>(v_opponent_key || '_hp'))::INT, 100);
            v_opponent_hp := LEAST(100, v_opponent_hp + v_drain_amount);
            v_game_state := jsonb_set(v_game_state, ARRAY[v_opponent_key || '_hp'], to_jsonb(v_opponent_hp));
            v_trap_activated := TRUE;
            v_log_entry := jsonb_build_object(
              'type', 'TRAP_ACTIVATED',
              ...
              'trap', v_trap_name,
              'effect', 'DRAIN',
              'value', v_drain_amount,
              'message', '💀 ' || v_trap_name || ' ativada! Roubou ' || v_drain_amount || ' HP do atacante!',
              'logged_at', v_logged_at,
              'timestamp', NOW()
            );
            v_battle_log := v_battle_log || v_log_entry;
...
-- ===== ATUALIZAR EFEITOS DAS CARTAS =====

-- Espelho Reverso: DOUBLE → SWAP_STATS
UPDATE cards 
SET effects = '[{"type": "SWAP_STATS", "value": 1, "description": "Troca ATK e DEF do monstro inimigo"}]'::jsonb
WHERE name = 'Espelho Reverso' AND card_type = 'TRAP';

-- Roubo de Energia: HEAL → DRAIN
UPDATE cards 
SET effects = '[{"type": "DRAIN", "value": 10, "description": "Rouba 10 HP do oponente"}]'::jsonb
WHERE name = 'Roubo de Energia' AND card_type = 'TRAP';