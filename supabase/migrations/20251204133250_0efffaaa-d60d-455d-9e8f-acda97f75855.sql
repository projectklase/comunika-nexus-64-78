-- Fix 3 battle bugs: draw mechanic, defeat condition, turn_number logic

-- 1. Update end_turn to add draw mechanic and defeat condition
...
-- 2. Update attack to NOT increment turn_number (only end_turn should do that)
...
BEGIN
  ...
  -- Monster cannot attack on the same turn it was summoned
  ...
  IF v_attacker->'active_effects' IS NOT NULL THEN
    ...
    IF COALESCE((v_attacker->>'current_hp')::INT, 0) <= 0 THEN
      ...
      -- DO NOT increment turn_number here - just switch turns
      UPDATE battles SET
        game_state = v_game_state,
  ...
  -- DO NOT increment turn_number here - attack doesn't end the turn automatically
  -- Player can choose to end turn manually or attack again if they have another action
  ...
  -- Attack successful but battle continues - DON'T switch turns automatically
  -- Player must click "End Turn" to pass
  UPDATE battles SET
    game_state = v_game_state,
    last_action_at = NOW(),
    updated_at = NOW()
