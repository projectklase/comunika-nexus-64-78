-- =============================================
-- Sistema de Cartas Limited Edition para Eventos
-- =============================================

-- 1. Criar tabela de eventos de cartas
CREATE TABLE IF NOT EXISTS public.card_events (
  ...
  show_in_collection BOOLEAN DEFAULT false,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);

-- 2. Adicionar coluna event_id na tabela cards
ALTER TABLE public.cards 
ADD COLUMN IF NOT EXISTS event_id UUID REFERENCES public.card_events(id) ON DELETE SET NULL;

-- 3. Criar índice para performance
CREATE INDEX IF NOT EXISTS idx_cards_event_id ON public.cards(event_id);
CREATE INDEX IF NOT EXISTS idx_card_events_active ON public.card_events(is_active, starts_at, ends_at);

-- 4. Enable RLS
ALTER TABLE public.card_events ENABLE ROW LEVEL SECURITY;

-- 5. RLS Policies para card_events

-- Superadmin pode gerenciar todos os eventos
CREATE POLICY "Superadmins can manage card events"
ON public.card_events
FOR ALL
USING (is_superadmin(auth.uid()));

-- Usuários autenticados podem ver eventos ativos
...
CREATE OR REPLACE FUNCTION update_card_events_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER update_card_events_timestamp
BEFORE UPDATE ON public.card_events
FOR EACH ROW
EXECUTE FUNCTION update_card_events_updated_at();

-- 7. Atualizar RPC open_card_pack para incluir cartas de eventos ativos
