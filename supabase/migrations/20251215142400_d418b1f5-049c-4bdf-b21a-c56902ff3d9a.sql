-- Adicionar suporte aos efeitos BURN, HEAL, FREEZE, DOUBLE para TRAPs e MONSTERs
-- BASEADO EXATAMENTE na migração funcional 20251215133414 - apenas ADIÇÕES, sem modificar código existente
-- Padrões mantidos: XP 10/3, overflow damage, summoning sickness, logs completos
...
BEGIN
  ...
  -- NEW: Check if attacker monster is FROZEN (cannot attack)
  ...
  IF v_attacker_monster->'effects' IS NOT NULL THEN
    ...
    LOOP
      ...
      -- NEW: DOUBLE - Dobra o efeito de BOOST
      ELSIF v_effect_type = 'DOUBLE' THEN
        v_boost_multiplier := 2.0;
      
      -- NEW: BURN - Dano adicional ao HP do oponente
      ...
      -- NEW: HEAL - Cura HP do atacante ao atacar
      ...
      -- NEW: FREEZE - Congela monstro inimigo por 1 turno
  ...
  IF jsonb_array_length(v_defender_traps) > 0 THEN
    ...
    IF v_trap->'effects' IS NOT NULL THEN
      ...
      LOOP
        ...
        -- NEW: BURN (Trap) - Causa dano direto ao HP do atacante
        ELSIF v_effect_type = 'BURN' THEN
          DECLARE
        ...
        -- NEW: HEAL (Trap) - Restaura HP do defensor
        ELSIF v_effect_type = 'HEAL' THEN
          DECLARE
        ...
        -- NEW: BOOST (Trap) - Armazena boost para próximo monstro (flag no game_state)
        ...
        -- NEW: DOUBLE (Trap) - Dobra o dano refletido ou causa dano dobrado
        ELSIF v_effect_type = 'DOUBLE' THEN
          DECLARE
            v_double_damage INT;
          BEGIN
            v_double_damage := FLOOR(v_damage * 0.5)::INT; -- 50% do dano original de volta
  ...
  -- ===== NEW: Apply BURN damage from attacker monster effect =====
  ...
  -- ===== NEW: Apply HEAL to attacker from monster effect =====
  ...
  -- ===== NEW: Apply FREEZE to defender monster =====
  ...
  IF v_winner_id IS NOT NULL THEN
    -- Grant XP to winner (10 XP - conforme solicitado)
    UPDATE profiles 
    SET total_xp = COALESCE(total_xp, 0) + 10,
        level_xp = COALESCE(level_xp, 0) + 10
    WHERE id = v_winner_id;
    
    -- Grant XP to loser (3 XP - conforme solicitado)
    UPDATE profiles 
