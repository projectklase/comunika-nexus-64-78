-- Drop restrictive policy that only allows viewing own entry
DROP POLICY IF EXISTS "Users can view own queue entry" ON battle_queue;

-- Create new policy allowing users to view queue entries in same school
CREATE POLICY "Users can view queue entries in same school" ON battle_queue
FOR SELECT USING (
  school_id IN (
...
-- Update join_battle_queue function to include SET search_path for security
