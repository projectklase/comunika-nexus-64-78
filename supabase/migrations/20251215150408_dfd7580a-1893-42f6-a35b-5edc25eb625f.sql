-- CORREÇÃO URGENTE: Bug na linha que salva HP após BURN de trap
-- A linha estava salvando v_trap_burn_damage ao invés de v_attacker_hp
  ...
  IF jsonb_array_length(v_defender_traps) > 0 THEN
    ...
    IF v_trap->'effects' IS NOT NULL THEN
      ...
      LOOP
        ...
        -- BURN (Trap) - DANO FIXO (value direto) - CORRIGIDO
        ELSIF v_effect_type = 'BURN' THEN
          DECLARE
          ...
          BEGIN
            ...
            -- CORREÇÃO: Salvar v_attacker_hp (não v_trap_burn_damage)
            v_game_state := jsonb_set(v_game_state, ARRAY[v_attacker_hp_key], to_jsonb(v_attacker_hp));
