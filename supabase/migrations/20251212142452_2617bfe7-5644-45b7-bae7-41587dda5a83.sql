...
-- Recriar função attack com suporte a valores decimais em efeitos
  ...
  IF v_attacker_monster->'effects' IS NOT NULL THEN
    ...
    LOOP
      ...
      IF v_effect_type = 'BOOST' THEN
        -- Handle both multiplier format (1.6) and percentage format (60)
        IF v_effect_value < 10 THEN
          -- Multiplier: 1.6 means +60%, so damage + damage * (1.6 - 1)
    ...
    IF v_trap->'effects' IS NOT NULL THEN
      ...
      LOOP
        ...
        IF v_effect_type = 'REFLECT' THEN
          -- Handle both multiplier and percentage formats
          DECLARE
            v_reflect_damage INT;
          BEGIN
            IF v_effect_value < 10 THEN
              v_reflect_damage := FLOOR(v_damage * (v_effect_value - 1))::INT;
            ELSE
        ...
        ELSIF v_effect_type = 'REDUCE' THEN
          -- Handle both multiplier and percentage formats
          IF v_effect_value < 10 THEN
            v_damage := FLOOR(v_damage * (2 - v_effect_value))::INT;
          ELSE
