...
BEGIN
  ...
  -- Determinar jogador e oponente (usar prefixos para construir chaves)
  ...
  -- Obter monstros em campo (usando chaves corretas: player1_field, player2_field)
  ...
  -- Obter HPs dos jogadores (usando chaves corretas: player1_hp, player2_hp)
  ...
  -- Atualizar HPs (usando chaves corretas)
  v_game_state := jsonb_set(v_game_state, ARRAY[v_opponent_key || '_hp'], to_jsonb(GREATEST(0, v_opponent_hp)));
  v_game_state := jsonb_set(v_game_state, ARRAY[v_player_key || '_hp'], to_jsonb(GREATEST(0, v_my_hp)));
  v_game_state := jsonb_set(v_game_state, '{battle_log}', v_battle_log);
