-- Correção SWAP_STATS: trocar ATK com current_hp (não apenas def)
-- O frontend mostra current_hp como HP visual, então precisamos atualizar esse campo
    ...
    IF v_trap->'effects' IS NOT NULL AND jsonb_array_length(v_trap->'effects') > 0 THEN
      ...
      CASE v_trap_effect_type
        ...
        WHEN 'SWAP_STATS' THEN
          -- CORREÇÃO: Troca ATK com current_hp (não apenas def)
          -- O frontend mostra current_hp como HP visual
